...
WHERE LOWER(UserLogin)=@UserLogin
...
where 
OpID in (select OpID from bluebin.BlueBinUserOperations where BlueBinUserID = (select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))
or
OpID in (select OpID from bluebin.BlueBinRoleOperations where RoleID in (select RoleID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))
...
select 
@Location,
(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @Scanner),
...
if not exists (select * from bluebin.BlueBinTraining where Active = 1 and BlueBinResourceID in (select BlueBinResourceID from bluebin.BlueBinResource where BlueBinResourceID  = @BlueBinResource))--
	...
	@Form3010,
	1, --Default Active to Yes
	(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @Updater),
	getdate()
...
set
	...
	Form3009 = @Form3009,
	Form3010 = @Form3010,
	BlueBinUserID = (select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @Updater),
	LastUpdated = getdate()
...
((select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin),@ActionType,@ActionName,@ActionID,getdate())
...
if not exists (select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin)
	BEGIN
...
BEGIN
      ...
      FROM [bluebin].[BlueBinUser] WHERE LOWER(UserLogin) = @UserLogin --(HASHBYTES('SHA1', @oldpwdHash))--@Password
...
BEGIN
      ...
      FROM [bluebin].[BlueBinUser] WHERE LOWER(UserLogin) = @UserLogin AND [Password] = (HASHBYTES('SHA1', @pwdHash))--@Password
...
BEGIN
      ...
      FROM [bluebin].[BlueBinUser] WHERE LOWER(UserLogin) = @UserLogin AND [Password] = (HASHBYTES('SHA1', @oldpwdHash))--@Password
...
where ActionType = 'Gemba' and 
		BlueBinUserID = (select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @Auditer) and 
			ActionID = (select convert(int,(convert(varchar,(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @Auditer))+convert(varchar,@ImageSourceIDPH))))
--if exists(select * from bluebin.[Image] where ImageSourceID = (select convert(int,(convert(varchar,(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))+convert(varchar,@ImageSourceIDPH))))
--	BEGIN
--	update [bluebin].[Image] set ImageSourceID = @GembaAuditNodeID where ImageSourceID = (select convert(int,(convert(varchar,(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))+convert(varchar,@ImageSourceIDPH))))
--	END
...
getdate(),  --Date
@Location,
(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @Auditer),
@AdditionalComments,
@PS_EmptyBins,
@PS_BackBins,
@PS_StockOuts,
...
VALUES 
(@ImageName,@ImageType,@ImageSource,(select convert(int,(convert(varchar,(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))+convert(varchar,@ImageSourceID)))),@Image,1,getdate(),getdate())

;
declare @ImageSourcePH int = (select convert(int,(convert(varchar,(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))+convert(varchar,@ImageSourceID))))
...
or 
(ImageSourceID = (select convert(int,(convert(varchar,(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))+convert(varchar,@ImageSourceIDPH)))))
...
where ActionType = 'Gemba' and 
		BlueBinUserID = (select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin) and 
			ActionID = (select convert(int,(convert(varchar,(select BlueBinUserID from bluebin.BlueBinUser where LOWER(UserLogin) = @UserLogin))+convert(varchar,@ImageSourceIDPH))))
